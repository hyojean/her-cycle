...
import { signInWithSocialProvider, type SocialProvider } from '../lib/oauth';
...
export default function HomePage() {
  const { configured, loading, isAuthenticated, displayName } = useAuthSession();
  const [lang, setLang] = useState<Lang>(() => loadLangFromStorage());
  const [loginProvider, setLoginProvider] = useState<SocialProvider | null>(null);
  ...
  const handleSocialLogin = async (provider: SocialProvider) => {
    ...
    try {
      ...
      setLoginError(error instanceof Error ? error.message : '로그인 중 문제가 발생했습니다.');
  ...
  return (
    <div className="home-container">
      ...
      <div className="main-greeting-card">
        ...
        {isAuthenticated ? (
          <>
            <h1 className="greeting-title">안녕하세요 {displayName}님</h1>
          ...
          <div className="login-prompt-area" style={{ width: '100%', padding: '0 var(--space-2)' }}>
            ...
            {loginError ? <p className="login-error-text">{loginError}</p> : null}

            <button
              className="social-login-btn"
              onClick={() => void handleSocialLogin('google')}
              disabled={!configured || loading || loginProvider !== null}
            >
              ...
              {loginProvider === 'google' ? 'Google 로그인 연결 중...' : 'Google 계정으로 로그인'}
            </button>
            <button
              className="social-login-btn"
            ...
            >
              ...
              {loginProvider === 'kakao' ? '카카오 로그인 연결 중...' : '카카오 계정으로 로그인'}
            </button>
          </div>
        )}
      </div>
